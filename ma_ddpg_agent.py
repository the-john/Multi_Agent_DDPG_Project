...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents):
        ...
        self.eps = 5.0
        self.eps_decay = 1/(300 * LEARN_NUM)  # set decay rate based on epsilon end target
        self.timestep = 0
    ...
    def step(self, state, action, reward, next_state, done, agent_number):
        ...
        # Learn, if enough samples are available in memory and the learning interval
...
class OUNoise:
    ...
    def sample(self):
        ...
        x = self.state
        dx = self.theta * (self.mu - x) + self.sigma * np.random.standard_normal(self.size)  # this change is fundamental to learning
        self.state = x + dx
        return self.state
